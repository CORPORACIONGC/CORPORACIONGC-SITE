...
- lo que la cita fuera de los tribunales, tras un filete dentro del mismo visual: la cifra a 32 px, el recuento por órgano en dos columnas y un desplegable con la lista —fecha, número enlazado a su fuente oficial (SINALEVI para la Procuraduría, SIGYD para la Contraloría, `tra.go.cr`, Nexus para las actas del Poder Judicial) y, cuando la hay, la cantidad de artículos de una misma sesión—. El pie se arma con las fuentes que esa sentencia sí tiene y nombra solo esas. Los pronunciamientos viven en `PRONUNCIAMIENTOS_QUE_CITAN` y también van al CSV.
