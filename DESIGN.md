...
La paleta es corta a propósito. **Burdeos (#6B1D3A)** es el color de marca y del botón principal; su versión clara (#8B2252) queda para el paso del cursor sobre el botón principal. Los titulares nunca llevan color: van en la tinta del tema. **Dorado (#C4A265)** es el hilo sobre fondo oscuro: bordes, líneas finas, iconos, numerales sobre burdeos, el foco, la selección y el botón con borde dorado. Como texto sobre blanco no alcanza el contraste AA (2,4:1), así que desde el 23-09-2026 no se usa de día ni como texto ni como filete decorativo: lo que de noche es dorado, de día va en el color de énfasis (`emphasis`, burdeos claro), y los filetes pasan a la tinta del tema. El rótulo del botón con borde dorado usa el **bronce oscuro (#7A5F2E)** en modo diurno y recupera el dorado en modo nocturno.
...
**El encabezado corrido** (`components/ui/RunningHead.tsx`, adoptado del Design System v2 el 23-09-2026). Cada sección abre con el nombre de la sección a la izquierda, en DM Sans medio a 17 px y en burdeos (dorado de noche y sobre las bandas oscuras), y, cuando aporta algo, una referencia a la derecha en versalitas de 11 px —«Figura 2» en las figuras de los artículos, «Sala Primera» en la jurisprudencia de los perfiles—, con un filete fino de tinta debajo. Las secciones no llevan numeral (decisión del 23-09-2026), y la cinta «Han confiado en nosotros» conserva su rótulo centrado. Sustituye al rótulo con rayita dorada. Nunca repite el titular que viene después, no se encierra en una pastilla y no lleva icono. La referencia va al 62 % de la tinta, que es lo mínimo que da 4,5:1 sobre blanco. Del mismo paquete se adoptaron los **gráficos planos** —una escalera de burdeos al 100, 55 y 28 % sobre un riel gris al 7 %, con puntas de 8 px, dorada de noche— y las **tablas editoriales**, con encabezados en versalitas de 11 px al 55 % de la tinta, filete inferior al 22 % y filetes de fila al 8 %. Los tokens están en `app/globals.css` (`--rule`, `--rule-strong`, `--rule-soft`, `--chart-1` a `--chart-3`, `--chart-track`). No se adoptaron la escala menor de titulares, las bandas planas, la cifra protagonista, el pie «Fuente ·» ni los botones sin cápsula.
...
**Membrete.** La composición de la hoja membretada de los escritos, llevada a la web (`components/ui/Membrete.tsx`): el sello GC a la izquierda y, a su lado, «CORPORACIÓN GC» en DM Sans medio, mayúscula y 0,2 em de interletraje (`membrete-name`), con «ABOGADOS · DERECHO PÚBLICO» debajo en gris espaciado (`membrete-caption`). Encabeza «Sobre nosotros», centrado y seguido de un filete fino. Es una pieza de marca: sus tamaños quedan fuera de la escala de texto a propósito.
...
**Explorador de áreas de práctica** (`components/sections/PracticeExplorer.tsx`). Reemplaza la cuadrícula de tarjetas con ícono. Las 32 áreas se ordenan en cinco grupos de práctica: Litigio y tribunales, Administración pública, Sectores regulados, Territorio y bienes públicos, y Cobertura complementaria. En escritorio, los grupos forman una columna de pestañas verticales en DM Sans ligera de 28–32 px (`group-tab`). El grupo activo toma el color de marca, su filete se traza y aparece una flecha. A la derecha, las áreas del grupo van en dos columnas de filas con filete, con el nombre en semibold y la descripción en 13 px, y el mismo trazo al pasar el cursor que la fila del equipo. El grupo cambia al pasar el cursor, con clic o con las flechas del teclado. Los paneles se apilan en la misma celda para que la sección no salte. En móvil, los grupos son una tira de pestañas deslizable. Las 32 áreas están siempre en el HTML inicial. Si se agrega un área nueva a `PRACTICE_AREA_PAGES`, hay que asignarla a un grupo en `GROUPS`; si no, cae por defecto en el grupo que corresponde a su prioridad. Ningún grupo pasa de ocho áreas (cuatro filas): como los paneles se apilan en la misma celda, un grupo más largo que el de Litigio, que se abre por defecto, deja un hueco debajo.

**Banda de clientes** (`components/sections/ClientLogos.tsx`). Debajo del hero, sobre la banda borgoña, «Han confiado en nosotros» en versalitas entre dos hilos dorados y una cinta de logos blancos que avanza a 80 s por vuelta y se detiene al pasar el cursor o recibir el foco. Los bordes se desvanecen con una máscara, nunca con franjas del color de fondo. Cada logo está recortado a su contenido (`public/images/clients/recorte`) y trae su proporción y su densidad de tinta, medidas sobre el archivo; `tamanoOptico()` (`lib/constants.ts`) los iguala en peso visual: un logo ancho baja de alto sin igualar el área y uno de trazo fino crece un poco. Nada se escala a mano. Con movimiento reducido, la cinta se vuelve una cuadrícula quieta de seis columnas (tres en móvil). La rejilla de clientes de Sobre nosotros usa los mismos tamaños.

**Artículos de la portada** (`components/sections/Publications.tsx`). Sobre la banda borgoña, un sumario de revista sin cajas. A la izquierda va el artículo más reciente: la firma, el título en ligera (`type-title`), el extracto en `type-lead` (tres líneas en móvil y cuatro desde tableta) y «Leer el artículo». A la derecha, separados por un filete vertical a igual distancia de ambas columnas (48 px), van los tres siguientes como lista (`role="list"`), cada uno con su título (`type-card-title`, hasta 46 caracteres por línea) y su firma. Al pasar el cursor o recibir el foco, el filete de la fila se traza en dorado, aparece una flecha y el retrato recupera el color completo: el mismo gesto de las filas del equipo y de las áreas. La firma lleva el retrato del autor en 4:5 con cabeza y hombros (56 px de ancho en el principal y 40 px en la lista), su nombre con el título académico, la fecha (`<time>`) y los minutos de lectura (a 220 palabras por minuto). La foto de oficina de don Óscar tiene su propio encuadre (`ENCUADRES`). Si el artículo oculta al autor (`authorVisible: false`), la firma solo muestra la fecha y los minutos. En la portada no hay etiquetas ni tipo de pieza, y solo aparecen artículos en español. El botón dice cuántos artículos hay («Ver los 38 artículos», calculado); en móvil baja al final de la lista. Los perfiles de los abogados usan el mismo componente con sus propios artículos, y ahí el botón dice «Ver todos los artículos».

**Folio de sentencia** (`components/sections/SentenciaFolio.tsx`). La sentencia destacada de la portada es una hoja de papel (`gc-papel`) apoyada sobre dos hojas más, como un expediente (`gc-pila`). La sombra tiene cuatro capas teñidas de burdeos, nunca negras. Arriba va el encabezado de la resolución, centrado: tribunal, número, fecha y hora, un filete fino, la categoría y el título. El título se escribe a 26–30 px en ligera, un encabezado de documento subordinado al titular de la sección. Debajo, dos voces separadas por un filete. A la izquierda, tres síntesis en prosa de la firma: el caso, el análisis y el impacto en la jurisprudencia (`sintesisPortada`). A la derecha, los pasajes literales de la Sala **en cursiva**, con comillas angulares en el color de énfasis colgadas al margen y su considerando debajo (`fragmentosPortada`, verificados por `fragmentosLiterales`). Al pie van la fórmula de redacción, el enlace al texto íntegro en Nexus y el enlace al análisis. Con más de una sentencia, debajo del folio aparece un índice (año, categoría, título y número) y la hoja cambia a la elegida; con una sola, el índice no se muestra. Las sentencias van en orden cronológico, y las columnas del índice se eligen para que ninguna fila quede coja: cuatro en escritorio y dos por dos en tableta con cuatro sentencias, tres con tres. La página de cada sentencia no reproduce el texto íntegro: enlaza a Nexus y cierra con una bibliografía (sentencia, precedentes citados y normativa, con enlaces a Nexus y SINALEVI).
...
- lo que la cita fuera de los tribunales, tras un filete dentro del mismo visual: la cifra a 32 px con su rango de años, el mismo par de gráficos de las resoluciones (columnas por año y barras por órgano), con su mismo comportamiento: cada año y cada órgano es un botón que abre la lista filtrada y marca en burdeos la parte que le toca al otro eje y un desplegable con la lista —fecha, número enlazado a su fuente oficial (SINALEVI para la Procuraduría, SIGYD para la Contraloría, `tra.go.cr`, Nexus para las actas del Poder Judicial) y, cuando la hay, la cantidad de artículos de una misma sesión—. El pie se arma con las fuentes que esa sentencia sí tiene y nombra solo esas. Los pronunciamientos viven en `PRONUNCIAMIENTOS_QUE_CITAN` y también van al CSV.
- la doctrina que la cita, al final del mismo visual: la cifra, cuántos son tesis, y la lista con año, título enlazado a su repositorio y autores. Vive en `DOCTRINA_QUE_CITA` y va al CSV. En los textos del sitio se nombran las fuentes públicas —Nexus, SINALEVI, los buscadores de la Contraloría, del Tribunal Registral y de la ARESEP, Kérwá y los repositorios universitarios—, nunca las bases internas de la oficina.

**El Código navegable** (`components/article/CodigoCPCA.tsx`, dentro del artículo del CPCA): la cifra de artículos a 44 px, un campo de búsqueda con filete inferior que acepta el número —«39»— o una palabra —«cautelar»—, los doce títulos como una línea de opciones y la lista de artículos, cada uno con su rótulo y el texto al desplegarlo. El índice viaja con la página para que el buscador lo lea; el texto de cada norma llega de `/datos/cpca.json` al abrirse. En los artículos que la jurisprudencia de la casa interpreta aparece el voto enlazado. Un componente insertado en un artículo lleva sus listas con `list-none`, que es la marca con que la prosa le quita la viñeta.

**Enlaces entre sentencias, áreas y artículos.** Cada sentencia declara en qué áreas de práctica aplicamos su criterio (`areas`) y con qué etiquetas de artículo se empareja (`temas`). Con eso, la página del área muestra «Sentencias que aplicamos en…», el artículo muestra «Sentencias comentadas sobre esta materia» —solo en español— y la sentencia cierra con «Dónde aplicamos este criterio». Las tres listas usan `SentenciasRelacionadas` o su gemelo de áreas, con el mismo trazo de las tarjetas de «Áreas relacionadas».
