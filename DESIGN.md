...
**Artículos de la portada** (`components/sections/Publications.tsx`). Sobre la banda borgoña, un sumario de revista sin cajas. A la izquierda va el artículo más reciente: la firma, el título en ligera (`type-title`), el extracto en `type-lead` cortado a cuatro líneas y «Leer el artículo». A la derecha, separados por un filete vertical, van los tres siguientes, cada uno con su título (`type-card-title`) y su firma. Al pasar el cursor, el filete de la fila se traza en dorado y aparece una flecha, el mismo gesto de las filas del equipo y de las áreas. La firma lleva el retrato del autor en 4:5 con cabeza y hombros (56 px de ancho en el principal y 40 px en la lista), su nombre con el título académico y los minutos de lectura (a 220 palabras por minuto). La foto de oficina de don Óscar tiene su propio encuadre (`ENCUADRES`). Si el artículo oculta al autor (`authorVisible: false`), la firma solo muestra la fecha y los minutos. En la portada no hay etiquetas ni tipo de pieza, y solo aparecen artículos en español. En móvil, «Ver todos los artículos» cierra la lista. Los perfiles de los abogados usan el mismo componente con sus propios artículos.
