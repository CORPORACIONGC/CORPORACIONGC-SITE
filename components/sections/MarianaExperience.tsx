...
import { MARIANA_PROFILE } from "@/lib/constants";
import { AnimatedEntry, StaggerContainer, StaggerItem } from "@/components/ui/AnimatedEntry";

export function MarianaExperience() {
  return (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[0.4fr_1fr] gap-12 lg:gap-20">
          ...
          <div>
            ...
            <AnimatedEntry delay={0.1}>
              <h2 className="font-display text-3xl md:text-5xl tracking-tighter leading-[1.05] text-cream">
                Experiencia e
                <br />
                <span className="text-burgundy-light">investigación</span>
              </h2>
            </AnimatedEntry>

            <AnimatedEntry delay={0.2}>
              <p className="mt-5 text-sm text-cream/50 leading-relaxed max-w-[35ch]">
                Desde la Procuraduría General hasta el litigio especializado en
                malpraxis médica y derecho a la salud.
              </p>
            </AnimatedEntry>
          </div>
          ...
          <StaggerContainer className="relative" stagger={0.1}>
            ...
            <div className="space-y-1">
              {MARIANA_PROFILE.career.map((exp, i) => (
                <StaggerItem key={i}>
                  <div
            ...
            <StaggerItem>
              <div className="relative pl-10 mt-8 pt-6 border-t border-cream/[0.08]">
                ...
                <div className="space-y-6">
                  {MARIANA_PROFILE.research.map((res, i) => (
                    <div key={i}>
            ...
            <StaggerItem>
              <div className="relative pl-10 mt-8 pt-6 border-t border-cream/[0.08]">
                <div className="text-[10px] tracking-[0.2em] uppercase text-cream/40 mb-4">
                  Presencia mediática y académica
                </div>
                <div className="space-y-4">
                  {MARIANA_PROFILE.mediaAppearances.map((item, i) => (
                    <div key={i} className="flex items-baseline gap-2">
                      <div className={`w-1 h-1 rounded-full ${i === 0 ? "bg-gold" : "bg-burgundy-light"} mt-1.5 shrink-0`} />
                      <div>
                        <span className="text-sm font-medium text-cream/75">{item.title}</span>
                        <p className="text-xs text-cream/45 mt-0.5">{item.description}</p>
                      </div>
                    </div>
                  ))}
                </div>
